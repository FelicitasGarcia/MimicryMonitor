...
# Paths configuration (repo root resolved from this script's location)
MIMICRY_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LLVM_DIR="$MIMICRY_DIR/llvm"
INPUTS_DIR="$MIMICRY_DIR/inputs"
TEMPS_DIR="$MIMICRY_DIR/work/temps"
BUILD_DIR="$LLVM_DIR/llvm-project/build"
CFG_OUTPUT_DIR="$MIMICRY_DIR/work/cfgs"
...
# Generate IR with clang-19 to match the LLVM 19 opt used for dot-cfg and defuse below.
# opt's dot-cfg printer renumbers metadata when it emits the .dot, so the .ll and .dot
# never share metadata IDs; LLVMProcessing.buildInstrLineMap instead maps each instruction
# to its source line by normalized instruction text, which only matches if both files are
# printed by the same LLVM version. Hence clang-19 + opt-19 throughout.
