/* chcon -- change security context of files
   ...
   main (int argc, char **argv)
   {
     ...
     while ((optc = getopt_long (argc, argv, "HLPRhvu:r:t:l:", long_options, nullptr)) != -1 )
     ...
     if (reference_file)
       {
         ...
         if (getfilecon (reference_file, &ref_context) < 0)
           error (EXIT_FAILURE, errno, _("failed to get security context of %s"), quoteaf (reference_file));
     ...
     else
       {
         ...
         if (0 < is_selinux_enabled ()
           ...
           error (EXIT_FAILURE, errno, _("invalid context: %s"), quote (specified_context));
       }
     ...
     if (recurse && preserve_root)
       {
         ...
         if (root_dev_ino == nullptr)
           error (EXIT_FAILURE, errno, _("failed to get attributes of %s"), quoteaf ("/"));
       }
     else
       {
