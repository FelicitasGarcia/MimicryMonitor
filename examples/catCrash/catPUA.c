   ...
   * Usually much faster than other versions of cat, the difference
   ...
   copy_cat (void)
   {
     ...
     for (bool some_copied = false; ; some_copied = true)
       switch (copy_file_range (input_desc, nullptr, STDOUT_FILENO, nullptr, copy_max, 0))
         {
         case 0:
           return some_copied;

         case -1:
           if (errno == ENOSYS || is_ENOTSUP (errno) || errno == EINVAL || errno == EBADF || errno == EXDEV || errno == ETXTBSY || errno == EPERM)
             return 0;
           error (0, errno, "%s", quotef (infile));
           return -1;
         }
   ...
   main (int argc, char **argv)
   {
     ...
     int c;
     while ((c = getopt_long (argc, argv, "benstuvAET", long_options, nullptr)) != -1)
     ...
     do
       {
         ...
         if (! (number || show_ends || show_nonprinting || show_tabs || squeeze_blank))
           ...
           {
             int copy_cat_status = out_isreg && S_ISREG (stat_buf.st_mode) ? copy_cat () : 0;
         ...
         else
           {
             ...
             idx_t bufsize;
             if (ckd_mul (&bufsize, insize, 4) || ckd_add (&bufsize, bufsize, outsize) || ckd_add (&bufsize, bufsize, LINE_COUNTER_BUF_LEN - 1))


               xalloc_die ();
             char *outbuf = xalignalloc (page_size, bufsize);

             ok &= cat (inbuf, insize, outbuf, outsize, show_nonprinting, show_tabs, number, number_nonblank, show_ends, squeeze_blank);
