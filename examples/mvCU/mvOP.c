...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, "bfint:uvS:TZ", long_options, nullptr)) != -1)
    {
      switch (c)
        {
        case 'b':
          ...
          break;
        case 'f':
          if (no_clobber || x.interactive == I_ASK_USER || x.interactive == I_UNSPECIFIED)
            x.interactive = I_ALWAYS_YES;
          break;
        case 'i':
          if (no_clobber || x.interactive == I_ALWAYS_YES || x.interactive == I_UNSPECIFIED)
         ...
         if (! no_clobber)
            {
              ...
              if (optarg)
                update_opt = XARGMATCH ("--update", optarg, update_type_string, update_type);
              if (update_opt == UPDATE_ALL)
                {
                  /* Default mv operation.  */
                  x.update = false;
                  if (x.interactive != I_ASK_USER && x.interactive != I_ALWAYS_YES)
                    x.interactive = I_UNSPECIFIED;
                }
  ...
  if (no_target_directory)
    {
      if (target_directory)
        error (EXIT_FAILURE, 0, _("cannot combine --target-directory (-t) and --no-target-directory (-T)"));
      if (2 < n_files)
        {
  ...
  else if (target_directory)
    {
      ...
      if (! target_dirfd_valid (target_dirfd))
        error (EXIT_FAILURE, errno, _("target directory %s"), quoteaf (target_directory));
    }
  else
    {
      char const *lastfile = file[n_files - 1];
      if (n_files == 2 && !x.exchange)
        x.rename_errno = (renameatu (AT_FDCWD, file[0], AT_FDCWD, lastfile, RENAME_NOREPLACE) ? errno : 0);
      if (x.rename_errno != 0)
        {
  ...
  if (make_backups && (x.exchange || x.interactive == I_ALWAYS_SKIP || x.interactive == I_ALWAYS_NO))

    {
      error (0, 0, _("cannot combine --backup with " "--exchange, -n, or --update=none-fail"));
      usage (EXIT_FAILURE);
    }

  x.backup_type = (make_backups ? xget_version (_("backup type"), version_control_string): no_backups);
      ...
      for (int i = 0; i < n_files; ++i)
        {
          ...
          char *dest_relname;
          char *dest = file_name_concat (target_directory, source_basename, &dest_relname);
