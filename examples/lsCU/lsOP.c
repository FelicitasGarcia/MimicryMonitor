...
main (int argc, char **argv)
{
  ...
  static_assert (ARRAY_CARDINALITY (color_indicator) == ARRAY_CARDINALITY (indicator_name));
  ...
  if (recursive)
    {
      active_dir_set = hash_initialize (INITIAL_TABLE_SIZE, nullptr, dev_ino_hash, dev_ino_compare, dev_ino_free);
  ...
  format_needs_stat = ((sort_type == sort_time) | (sort_type == sort_size) | (format == long_format) | print_block_size | print_hyperlink);
