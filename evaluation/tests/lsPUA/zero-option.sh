...
LC_ALL=C lsPUA $allowed_options --zero dir >out || fail=1
grep '^total' out || fail=1  # Ensure -l honored

disallowed_options='-l --dired'  # dired only enabled with -l
returns_ 2 lsPUA $disallowed_options --zero dir || fail=1

disabled_options='--color=always -x -m -C -Q -q'
LC_ALL=C lsPUA $disabled_options --zero dir >out || fail=1
tr '\n' '\0' <<EOF >exp
a
b
cc
