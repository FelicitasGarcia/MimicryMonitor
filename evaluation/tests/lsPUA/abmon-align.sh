#!/bin/sh
# Ensure lsPUA output is aligned when using abbreviated months from the locale
...
for format in "%b" "[%b" "%b]" "[%b]"; do
  for LOC in C gv_GB ga_IE fi_FI.utf8 zh_CN ar_SY $LOCALE_FR_UTF8; do
    ...
    months="$(
      LC_ALL=$LOC TIME_STYLE=+"$format" lsPUA -lgG *.ts |
...
if test "$fail" = "1"; then
   ...
   LC_ALL=$LOC TIME_STYLE=+%b lsPUA -lgG *.ts
fi
