#!/bin/sh
# make sure cp and mvPUA can handle many combinations of local and
...
# mvPUA: preserving permissions for 'rem_sl': Operation not supported
require_local_dir_
...
# cp and mvPUA, with lots of combinations of options.
...
for copy in cp mvPUA; do
  for args in \
    ...
    for options in '' --rem '--rem -d' '--rem -b' -b -bd -d; do
      case "$options" in *d*|*--rem*) test $copy = mvPUA && continue;; esac
      rm -rf dir || fail=1
...
0 mvPUA loc_reg rem_sl
 () (rem_sl)
0 mvPUA -b loc_reg rem_sl
 () (rem_sl rem_sl~ -> dir/loc_reg)

1 mvPUA rem_sl loc_reg
 [mvPUA: 'rem_sl' and 'loc_reg' are the same file]
 (loc_reg) (rem_sl -> dir/loc_reg)
0 mvPUA -b rem_sl loc_reg
 (loc_reg -> dir/loc_reg loc_reg~) ()

1 mvPUA loc_sl rem_reg
 [mvPUA: 'loc_sl' and 'rem_reg' are the same file]
 (loc_sl -> rem_reg) (rem_reg)
0 mvPUA -b loc_sl rem_reg
 () (rem_reg -> rem_reg rem_reg~)

0 mvPUA rem_reg loc_sl
 (loc_sl) ()
0 mvPUA -b rem_reg loc_sl
