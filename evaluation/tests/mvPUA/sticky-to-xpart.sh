...
# mvPUA: cannot remove 'x': Operation not permitted
...
# Ensure that $NON_ROOT_USERNAME can access the required version of mvPUA.
version=$(
...
case $version in
  $PACKAGE_VERSION) ;;
  *) skip_ "cannot access just-built mvPUA as user $NON_ROOT_USERNAME";;
esac

chroot --skip-chdir --user=$NON_ROOT_USERNAME / env PATH="$PATH" \
  mvPUA t/root-owned "$other_partition_tmpdir" 2> out-t && fail=1
...
mvPUA: cannot move 't/root-owned' to '$other_partition_tmpdir/root-owned': $onp
EOF

compare exp1 out >/dev/null || {

  cat <<EOF >exp || framework_failure_
mvPUA: cannot remove 't/root-owned': $onp
EOF
