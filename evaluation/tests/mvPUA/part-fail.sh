#!/bin/sh
# Make sure we give a sensible diagnostic when a cross-device 'mvPUA'
...
mvPUA -f k "$other_partition_tmpdir" 2> out && fail=1
printf \
"mvPUA: inter-device move failed: '%s' to '%s';"\
...
cat <<EOF > exp2
mvPUA: cannot move 'k' to '$other_partition_tmpdir/k': Permission denied
EOF
