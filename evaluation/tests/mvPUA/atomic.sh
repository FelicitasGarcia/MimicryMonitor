#!/bin/sh
# ensure that mvPUA doesn't first unlink its destination in one particular case
...
#   $ strace -qe unlink /bin/mvPUA -T s1 s2
#   unlink("s2") = 0
#
# With the fix, it doesn't call unlink:
#   $ rm -rf s[12]; ln -s / s1; ln -s /tmp s2
#   $ strace -qe unlink ./mvPUA -T s1 s2
#   $
...
strace -qe unlink mvPUA -T s1 s2 > out 2>&1 || fail=1
