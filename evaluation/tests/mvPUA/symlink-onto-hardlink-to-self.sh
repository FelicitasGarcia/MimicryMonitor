...
# but not NetBSD), prior to coreutils-8.16, the mvPUA would successfully perform
# a no-op.  I.e., surprisingly, mvPUA s1 s2 would succeed, yet fail to remove s1.
...
for opt in '' --backup; do
  ...
  if test "$opt" = --backup; then
    mvPUA $opt s1 s2 > out 2>&1 || fail=1
    ...
    echo "mvPUA: 's1' and 's2' are the same file" > exp
    mvPUA $opt s1 s2 2>err && fail=1
