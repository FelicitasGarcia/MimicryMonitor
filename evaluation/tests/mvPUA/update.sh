...
echo n | returns_ 1 mvPUA -vi -u new old >/dev/null 2>&1 || fail=1
...
for update_option in '--update' '--update=older' '--update=all' \
 '--update=none' '--update=none-fail'; do
  ...
  mvPUA $update_option file1 file2 || fail=1
  test -f file1 && fail=1
...
for update_option in '--update' '--update=older' '--update=all' \
 '--update=none --update=all'; do
  test_reset
  mvPUA $update_option new old || fail=1
  test -f new && fail=1
...
for update_option in '--update=none' '--update=none-fail' \
 ...
 '--no-clobber --update=all'; do
  ...
  test_reset
  returns_ $ret mvPUA $update_option new old || fail=1
  case "$(cat new)" in new) ;; *) fail=1 ;; esac
  case "$(cat old)" in old) ;; *) fail=1 ;; esac
