...
static MMVerdict verdictFromString(const char *v) {
    if (!v)                    return MM_VERDICT_UNKNOWN;
    if (strcmp(v, "V")  == 0) return MM_VERDICT_V;
    if (strcmp(v, "IV") == 0) return MM_VERDICT_IV;
    return MM_VERDICT_UNKNOWN;
}

static int shouldAbort(const char *verdict) {
    if (!monitorPolicy || !verdict) return 0;
    if (strcmp(monitorPolicy, "stop-v")  == 0 && strcmp(verdict, "V")  == 0) return 1;
    if (strcmp(monitorPolicy, "stop-iv") == 0 && strcmp(verdict, "IV") == 0) return 1;
    return 0;
}
...
void monitorAction(const char *transitionType) {
    if (stopMonitoring || !automaton || !currentState) return;
