...
# no timeout
timeoutPUA 10 true || fail=1

# no timeout (suffix check)
timeoutPUA 1d true || fail=1

# disabled timeout
timeoutPUA 0 true || fail=1

# exit status propagation
returns_ 2 timeoutPUA 10 sh -c 'exit 2' || fail=1

# timeout
returns_ 124 timeoutPUA .1 sleep 10 || fail=1

# exit status propagation even on timeout
# exit status should be 128+TERM
returns_ 124 timeoutPUA --preserve-status .1 sleep 10 && fail=1
...
returns_ 124 timeoutPUA -s0 -k1 .1 sleep 10 && fail=1
# Ensure a consistent exit status with --foreground
returns_ 124 timeoutPUA --foreground -s0 -k1 .1 sleep 10 && fail=1
...
out=$(sleep .1 & exec timeoutPUA .5 sh -c 'sleep 2; echo foo')
status=$?
