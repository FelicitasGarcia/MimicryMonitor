...
# mvPUA should fail for the same name, or separate hardlinks as in
# both cases rename() will do nothing and return success.
# One could unlink(src) in the hardlink case, but that would
# introduce races with overlapping mvPUA instances removing both hardlinks.

for dest in $ff $ff2; do
  # This mvPUA command should exit nonzero.
  mvPUA $ff $dest > out 2>&1 && fail=1

  printf "mvPUA: '$ff' and '$dest' are the same file\n" > exp
