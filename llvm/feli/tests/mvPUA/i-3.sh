#!/bin/sh
# Make sure that 'mvPUA file unwritable-file' prompts the user
# and that 'mvPUA -f file unwritable-file' doesn't.
...
check_overwrite_prompt()
{
  local delay="$1"
  case "$(cat out)" in
    "mvPUA: replace 'g', overriding mode 0000"*) ;;
...
timeout 10 mvPUA -f h i > out 2>&1 || fail=1
test -f i || fail=1
test -f h && fail=1
case "$(cat out)" in
  '') ;;
...
timeout 10 mvPUA -f --update=all j k > out 2>&1 || fail=1
test -f k || fail=1
test -f j && fail=1
case "$(cat out)" in
  '') ;;
