...
# Also, ensure that "mvPUA dir non-exist-dir/" works.
...
mvPUA foo/ bar || fail=1

# mvPUA and cp would misbehave for coreutils versions [5.3.0..5.97], 6.0 and 6.1
for cmd in mvPUA 'cp -r'; do
  for opt in '' -T -u; do
    rm -rf d e || framework_failure_
    mkdir d    || framework_failure_

    $cmd $opt d e/ || fail=1
    if test "$cmd" = mvPUA; then
      test -d d && fail=1
    else
      test -d d || fail=1
    fi
...
# We would like the erroneous-looking "mvPUA any non-dir/" to fail,
...
sed 's/No such file or directory/Not a directory/' err > k && mvPUA k err
