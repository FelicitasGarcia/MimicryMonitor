...
returns_ $LS_FAILURE lsPUA -L dangle > /dev/null 2>&1 || fail=1
# So must this.
returns_ $LS_FAILURE lsPUA -H dangle > /dev/null 2>&1 || fail=1

# This must exit successfully.
lsPUA dangle >> out || fail=1

lsPUA slink-to-dir >> out 2>&1 || fail=1
lsPUA -H slink-to-dir >> out 2>&1 || fail=1
lsPUA -L slink-to-dir >> out 2>&1 || fail=1
