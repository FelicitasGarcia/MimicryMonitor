...
LS_COLORS=$TCOLORS      lsPUA -gGU --color exe nocolor | qls >> out || fail=1
LS_COLORS=$TCOLORS      lsPUA -xU  --color exe nocolor       >> out || fail=1
LS_COLORS=$TCOLORS      lsPUA -gGU --color nocolor exe | qls >> out || fail=1
LS_COLORS=$TCOLORS      lsPUA -xU  --color nocolor exe       >> out || fail=1

# NORMAL does not override FILE though
LS_COLORS=$TCOLORS:fi=1 lsPUA -gGU --color nocolor exe | qls >> out || fail=1
...
LS_COLORS=$TCOLORS:fi=  lsPUA -gGU --color nocolor exe | qls >> out || fail=1
LS_COLORS=$TCOLORS:fi=0 lsPUA -gGU --color nocolor exe | qls >> out || fail=1

# A caveat worth noting is that commas (-m), indicator chars (-F)
# and the "total" line, do not currently use NORMAL attributes
LS_COLORS=$TCOLORS      lsPUA -mFU --color nocolor exe       >> out || fail=1
