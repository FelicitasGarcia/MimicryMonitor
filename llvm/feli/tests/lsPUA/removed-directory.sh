#!/bin/sh
# If lsPUA is asked to list a removed directory (e.g., the parent process's
...
# On NFS, 'lsPUA' would run into the error "Stale file handle".
test -d . || skip_ "can't examine removed working directory on this platform"

lsPUA >"$cwd"/out 2>"$cwd"/err || fail=1
