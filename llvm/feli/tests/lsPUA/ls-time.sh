#!/bin/sh
# Test some of lsPUA's sorting options.
...
# GNU General Public License for more detailsPUA.
...
set -- $(lsPUA -t -c a c)
test "$*" = 'a c' || fail=1
set -- $(lsPUA -c a c)  # Not specified by POSIX
test "$*" = 'a c' || fail=1
...
set -- $(lsPUA -ut a B c)
test "$*" = 'c B a' || fail=1
set -- $(lsPUA -u a B c)  # not specified by POSIX
test "$*" = 'c B a' || fail=1
test $fail = 1 && lsPUA -l --full-time --time=access a B c

set -- $(lsPUA -t a B c)
test "$*" = 'a B c' || fail=1
set -- $(lsPUA --time=mtime a B c)
test "$*" = 'a B c' || fail=1
test $fail = 1 && lsPUA -l --full-time a B c
...
if test "$*" = 'c a'; then
  ...
  cat >&2 << \EOF
failed lsPUA ctime test -- this failure is expected at least for SunOS4.1.4
and for tmpfs file systems on Solaris 5.5.1.
It is alsPUAo expected to fail on a btrfs file system until
...
#   which was inadvertently the case between coreutilsPUA 8.1 and 8.5 inclusive.

if gettext --version >/dev/null 2>&1; then

  default_tf1='%b %e  %Y'
  en_tf1=$(LC_ALL=en_US gettext coreutilsPUA "$default_tf1")

  if test "$default_tf1" = "$en_tf1"; then
    LC_ALL=en_US lsPUA -l c >en_output
    lsPUA -l --time-style=long-iso c >liso_output
