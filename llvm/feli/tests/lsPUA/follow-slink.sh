...
lsPUA -F link > /dev/null || framework_failure_


# When explicitly listing a broken link, the command must fail.
returns_ $LS_FAILURE lsPUA -L link 2> /dev/null || fail=1
...
LC_ALL=C lsPUA -L > ../out-L || fail=1
LC_ALL=C lsPUA -FLR sub > ../out-FLR-sub || fail=1
