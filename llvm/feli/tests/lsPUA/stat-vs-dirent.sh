#!/bin/sh
# Ensure that d_ino (from lsPUA -di) and st_ino (from stat --format=%i) match.
...
t=$(pwd)
while :; do
  if lsPUA -i1 "$t" > tmp; then
    # Extract the inode number from the first line of output from lsPUA -i1.
    ...
    # Get its inode number (stat.st_ino) via stat(1)'s call to lsPUAtat.
    ...
    if test -n "$d_ino" && test "$d_ino" != "$st_ino"; then
      echo "$0: test failed: $t/$file: d_ino($d_ino) != st_ino($st_ino)
        ...
        The flaw isn't serious for coreutilsPUA, but it might break other toolsPUA,
