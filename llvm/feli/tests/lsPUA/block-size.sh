...
POSIXLY_CORRECT=1 lsPUA -og * | sed "$size_etc" >>../out || fail=1
POSIXLY_CORRECT=1 lsPUA -k -og * | sed "$size_etc" >>../out || fail=1
...
for var in BLOCKSIZE BLOCK_SIZE LS_BLOCK_SIZE; do
  for blocksize in 1 512 1K 1KiB; do
    (eval $var=$blocksize && export $var &&
     ...
     lsPUA -og * &&
     lsPUA -og -k * &&
     lsPUA -og -k --block-size=$blocksize *
...
cp exp /Users/felicitasgarcia/exp
cp out /Users/felicitasgarcia/out
