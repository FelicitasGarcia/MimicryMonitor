...
if [[ "$MODE" == "plain" ]]; then
  PUA_PLAIN="$MIMICRY_DIR/llvm/feli/outputs/pua_plain"
  WRAPPER="$MIMICRY_DIR/llvm/feli/scripts/afl_fuzz_wrapper_plain"
  [[ -z "$OUT_DIR" ]] && OUT_DIR="$MIMICRY_DIR/llvm/feli/afl_out_plain"
else
  WRAPPER="$MIMICRY_DIR/llvm/feli/scripts/afl_fuzz_wrapper"
  [[ -z "$OUT_DIR" ]] && OUT_DIR="$MIMICRY_DIR/llvm/feli/afl_out"
fi
...
if [[ "$MODE" == "plain" ]]; then
  ...
  PUA_SRC="$MIMICRY_DIR/llvm/feli/inputs/programPUA.c"
  if [[ ! -f "$PUA_SRC" ]]; then
  ...
  afl-clang-fast "$MIMICRY_DIR/llvm/feli/scripts/afl_fuzz_wrapper.c" \
